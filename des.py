...
def main():
    mode = input("Pilih mode (1 untuk Enkripsi, 2 untuk Dekripsi): ")
    key, subkeys = generate_key()
    iv = generate_iv()

    if mode == '1':
        plaintext = input("Masukkan plaintext: ").encode()
        ciphertext = des_cfb_encrypt(plaintext, subkeys, iv)
        print(f"Ciphertext: {ciphertext.hex()}")
        print(f"Kunci (Hex): {key.hex()}")
        print(f"IV (Hex): {iv.hex()}")
    elif mode == '2':
        key_input = input("Masukkan kunci (Hex): ")
        iv_input = input("Masukkan IV (Hex): ")
        key = bytes.fromhex(key_input)
        key_bin = ''.join(f'{byte:08b}' for byte in key)
        subkeys = key_generator(key_bin)
        iv = bytes.fromhex(iv_input)

        ciphertext_input = input("Masukkan ciphertext (Hex): ")
        ciphertext = bytes.fromhex(ciphertext_input)

        plaintext = des_cfb_decrypt(ciphertext, subkeys, iv)
        print(f"Plaintext: {plaintext.decode('utf-8', 'ignore')}")
    else:
